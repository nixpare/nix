...
const (
	OkayLogFormat = "%-16s - %-4s %-65s %s %d %10.3f MB - (%6d ms) \u279C %s (%s) via %s\n"
	BadLogFormat  = "%-16s - %-4s %-65s %s %d %10.3f MB - (%6d ms) \u279C %s (%s) via %s \u279C %s\n"
)
...
func (h handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case metrics.Code < 400:
		...
		route.Router.Logger.Log(LogLevelInfo, fmt.Sprintf(
			OkayLogFormat,
			route.RemoteAddress,
			route.R.Method,
			route.logRequestURI,
	...
	case metrics.Code >= 400 && metrics.Code < 500:
		route.Router.Logger.Log(LogLevelWarning, fmt.Sprintf(
			BadLogFormat,
			route.RemoteAddress,
			route.R.Method,
			route.logRequestURI,
	...
	default:
		route.Router.Logger.Log(LogLevelError, fmt.Sprintf(
			BadLogFormat,
			route.RemoteAddress,
			route.R.Method,
			route.logRequestURI,
...
func (route *Route) serveError() {
	...
	err := route.errTemplate.Execute(route.W, struct {
		Code    int
		Message string
	}{Code: route.W.code, Message: route.errMessage})
	if err != nil {
		route.Router.Logger.Log(LogLevelError, fmt.Sprintf("Error serving template file: %v\n", err))
		return
	}
}
